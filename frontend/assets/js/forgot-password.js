// File: frontend/assets/js/forgot-password.js
// 🔒 Secure Forgot Password Page JavaScript - CSP Compliant
...
(function() {
    ...
    function initializeButtonState() {
        ...
        if (buttonText) {
            buttonText.textContent = 'Send Reset Email';
        }
        ...
        if (forgotForm) {
            forgotForm.addEventListener('submit', async function(e) {
                ...
                if (window.logAuthEvent) {
                    window.logAuthEvent('forgot_password_form_submission', {
                        action: 'form_submit',
                        page: 'forgot-password',
                ...
                if (!emailValid) {
                    ...
                    if (window.logSecurityEvent) {
                        window.logSecurityEvent('forgot_password_validation_failure', {
                ...
                // Environment-aware debug logging
                if (window.AppConfig?.isDebug()) {
                    console.log('Calling Auth.requestPasswordReset', { email: '[EMAIL_REDACTED]' });
                }

                try {
                    // Use Auth class to handle the request
                    if (window.Auth && window.Auth.requestPasswordReset) {
                        const result = await window.Auth.requestPasswordReset(email);

                        // Environment-aware debug logging
                        if (window.AppConfig?.isDebug()) {
                            console.log('Auth.requestPasswordReset completed', { success: result?.success });
                        }
                    } else {
                        ...
                        if (window.logError) {
                            window.logError('Auth system not available on forgot password page', {
                                page: 'forgot-password'
                            });
                        }
                    }
                    ...
                    if (window.logError) {
                        window.logError('Error in forgot password submission', {
                            error: error.message,
                            page: 'forgot-password'
                        });
                    }
                    ...
                    if (buttonText) buttonText.textContent = 'Send Reset Email';
    ...
    async function initializeConfiguration() {
        try {
            ...
            // Environment-aware logging
            if (window.AppConfig?.isDebug()) {
                console.log('✅ Configuration loaded for forgot password page');
            } else {
                if (window.logInfo) {
                    window.logInfo('Configuration loaded for forgot password page', {
                        page: 'forgot-password'
                    });
                }
            }
            ...
            // Environment-aware error logging
            if (window.AppConfig?.isDebug()) {
                console.warn('⚠️ Config initialization failed:', error);
            }
    ...
    function setupSecurityMonitoring() {
        window.addEventListener('error', function(e) {
            ...
            if (window.logError) {
                window.logError('JavaScript Error on Forgot Password Page', {
                    message: e.message,
                    filename: e.filename,
                    lineno: e.lineno,
                    page: 'forgot-password',
        ...
        window.addEventListener('unhandledrejection', function(e) {
            ...
            if (window.logError) {
                window.logError('Unhandled Promise Rejection on Forgot Password Page', {
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        if (window.logInfo) {
            window.logInfo('Forgot Password page initialized', {
                page: 'forgot-password',
